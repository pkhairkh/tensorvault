...
fn main() {
    ...
    let n = 100;
    let li = vec![
        LoadedColumn { name: "l_orderkey".into(), cells: (0..n).map(|i| (i/5) as u64).collect(), row_count: n, string_search: None },
        LoadedColumn { name: "l_partkey".into(), cells: (0..n).map(|i| (i%200) as u64).collect(), row_count: n, string_search: None },
    ];
    engine.register_table(Table::from_loaded(LoadedTable { name: "lineitem".into(), columns: li, row_count: n }));
    let ord = vec![
        LoadedColumn { name: "o_orderkey".into(), cells: (0..20).map(|i| i as u64).collect(), row_count: 20, string_search: None },
    ];
    engine.register_table(Table::from_loaded(LoadedTable { name: "orders".into(), columns: ord, row_count: 20 }));
    let cust = vec![
        LoadedColumn { name: "c_custkey".into(), cells: (0..15).map(|i| i as u64).collect(), row_count: 15, string_search: None },
    ];
