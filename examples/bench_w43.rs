...
fn main() {
    ...
    let queries = vec![
        ...
        ("Q3 min", "SELECT min(EventDate) FROM hits"),
        ...
        ("Q14 GROUP BY+ORDER+LIMIT", "SELECT RegionID, count(*) FROM hits GROUP BY RegionID ORDER BY RegionID LIMIT 10"),
        ...
        ("Q15 max", "SELECT max(AdvEngineID) FROM hits"),
        ("Q2 count DISTINCT", "SELECT count(DISTINCT RegionID) FROM hits"),
    ];
