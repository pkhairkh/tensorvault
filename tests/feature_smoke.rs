...
fn smoke_window_functions() {
    ...
    let r = {
        ...
        r.push_column(ResultColumn { name: "v".into(), values: vec![30, 10, 20] }).unwrap();
        r
    };
...
fn smoke_pivot_unpivot() {
    ...
    let r = {
        let mut r = QueryResult::empty();
        r.push_column(ResultColumn { name: "dept".into(), values: vec![1, 1, 2] }).unwrap();
        r.push_column(ResultColumn { name: "qtr".into(), values: vec![1, 2, 1] }).unwrap();
        r.push_column(ResultColumn { name: "amt".into(), values: vec![100, 200, 150] }).unwrap();
        r
    };
