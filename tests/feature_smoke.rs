...
fn smoke_window_functions() {
    ...
    let r = {
        ...
        r.push_column(ResultColumn { name: "v".into(), values: vec![30, 10, 20], string_values: None }).unwrap();
        r
    };
...
fn smoke_pivot_unpivot() {
    ...
    let r = {
        let mut r = QueryResult::empty();
        r.push_column(ResultColumn { name: "dept".into(), values: vec![1, 1, 2], string_values: None }).unwrap();
        r.push_column(ResultColumn { name: "qtr".into(), values: vec![1, 2, 1], string_values: None }).unwrap();
        r.push_column(ResultColumn { name: "amt".into(), values: vec![100, 200, 150], string_values: None }).unwrap();
        r
    };
